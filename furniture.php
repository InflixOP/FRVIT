<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Equipment</title>
    <link rel="stylesheet" href="furniture.css">
</head>
<body>
    <img id="logo"
    src="logo.svg" alt="">
    <div id="topblock">
        <span><a href="student_dashboard.php" id="homee">Home</a></span>
        <span><a href="logout_student.php" id="logoutt">Logout</a></span>
    </div>
    <div id="box" >
        <span id="heading"> <p><strong>Damaged Furniture</strong></p> </span>
    </div>
    <form action="furniture.php" method="post">
        &nbsp;&nbsp;<select name="furniture" class="name" id="domain">
            <option value="none" selected disabled hidden >Select an Option</option>
            <option value="Bed">Bed</option>
            <option value="Chair">Chair</option>
            <option value="studytable">Study table</option>
            <option value="Window">Window</option>
            <option value="Door">Door</option>
            <option value="Wardrobe">Wardrobe</option>
            <option value="Wardrobe">Mirror</option>
            <option value="Others">Others</option>
        </select><br><br>


    <input type="submit" id="con" value="Confirm">
    </form>

<?php
    $server = "localhost";
    $usernam = "root";
    $password = "";
    error_reporting(E_ERROR | E_PARSE);

    $con = mysqli_connect($server, $usernam, $password);
    $furniture=$_POST["furniture"];
    if (isset($_POST["furniture"])) {
        $sql2 = "SELECT * FROM `student_data`.`signup_student` WHERE `login`=1";
        $result = mysqli_query($con, $sql2);
        while ($row = mysqli_fetch_assoc($result)) {
            $regno=$row["regno"];
            $hostel=$row["block"];
            $name=$row["name"];
            $room=$row["room"];
            $sql1="INSERT INTO `student_data`.`$regno` (`complain`,`status`) VALUES('$furniture','Pending')"; 
            
                if ($con->query($sql1) == true) {
                $sql3="INSERT INTO `admin_data`.`$hostel` (`name`,`regno`,`room`,`complain`,`status`) VALUES('$name','$regno','$room','$furniture','Pending')";
                if ($con->query($sql3) == true) {
                    header("Location:student_dashboard.php");  
            }
            else {
               echo "ERROR: $sql3 <br> $con->error";
            }}}}
        
        
    ?>

</body>
</html>